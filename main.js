...
const addBtns = () => {
  let printBtns = `<button id="afc" class="btn btn-outline-primary">AFC</button>`;
  printBtns += `<button  id="nfc" type="button" class="btn btn-outline-secondary">NFC</button>`;
  printBtns += `<button id="playoffs"type="button" class="btn btn-outline-success">Playoffs</button>`;
  printBtns += `<button id="all" type="button" class="btn btn-outline-danger">All</button>`;
printToDom(printBtns,'btnsArea');
};


// add jumbotron to the div id of top
const addJumbo = () => {
  const printJumbo = `<div class="jumbotron">
  <h1 class="display-4">GAME TIME</h1>
  <p class="lead">ARE YOUR READY FOR SOME FOOTBALL?</p>
  <hr class="my-4">
  <p>The ROAD TO THE SUPER STARTS HERE</p>
  <button id="btn1" class="btn btn-primary btn-lg" role="button">🏈 GAME TIME 🏈</button>
</div>`;
printToDom(printJumbo,'top');
}
addJumbo();
...
printToDom(allTeams,'middle');
};


const showCards = () => {  
$("#btn1").on('click',addBtns);
$("#btn1").on('click',addCards);
};
showCards();
...
//   if (btnType = afc) {
//     console.log("AFC Wins");
//   }
// };

// const btn2Clicked = () => {
//   $("#btn2").on('click',btnEvents);
// }
// btn2Clicked();